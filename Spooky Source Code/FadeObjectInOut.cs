...
public class FadeObjectInOut : MonoBehaviour
{
	...
	IEnumerator FadeSequence (float fadingOutTime)
	{
		...
		if (fadingOut)
		{
			...
			if(death_on_fade){
				GameObject the_spawner = GameObject.Find ("spawner");
				spawner_behavior spawn_script = the_spawner.GetComponent<spawner_behavior>();
				if(gameObject.GetComponent<creep_behaviour>().is_last){
					spawn_script.total_spawn = 0;
					spawn_script.level += 1;
					spawn_script.spawn_limit += 1;
				}
				//Remove the object from the list of targets in the room
				Doorway_Behaviour the_door = this.gameObject.GetComponent<creep_behaviour>().cur_door.GetComponent<Doorway_Behaviour>();
				the_door.Remove_From_List (this.gameObject);
